...
st.markdown(
    ...
    div[data-testid="stVerticalBlockBorderWrapper"]{
        border:1px solid rgba(8,119,201,.12)!important;
...
def get_official_facilities(park_name, facility_type):
    """
    公式一覧HTMLから detail/数字/ の個別ページだけを取得する。
    一覧・トップ・メニューページは候補に含めない。
    """
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        absolute_url = urllib.parse.urljoin(list_url, href)
        ...
        for attr in ["aria-label", "title"]:
            if anchor.get(attr):
                parts.append(anchor.get(attr))

        for selector in [
            "[class*='name']",
            "[class*='title']",
            "h2", "h3", "h4", "strong", "dt", "dd", "p", "span"
        ]:
            for node in anchor.select(selector):
                text = node.get_text(" ", strip=True)
                if text:
                    parts.append(text)

        plain = anchor.get_text(" ", strip=True)
        if plain:
            parts.append(plain)
...
# タブ
all_tab, search_tab, favorite_tab = st.tabs(
    [
        "✨ おすすめ",
        "🔎 施設検索",
...
def show_facility_cards(frame, key_prefix):
    ...
    for index, row in frame.iterrows():
        ...
        type_icon = TYPE_ICONS.get(row["type"], "📍")
        official_info = match_official_facility(
            park_name,
            row["type"],
            row["name_ja"],
        )
